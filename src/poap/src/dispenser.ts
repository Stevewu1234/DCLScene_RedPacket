...
import { playerStatus } from "src/playerData";
...
export class Dispenser extends Entity {
  ...
  constructor({
    ...
    this.addComponent(new GLTFShape("src/poap/models/poap/POAP_dispenser.glb"));
    ...
    button.addComponent(new GLTFShape("src/poap/models/poap/POAP_button.glb"));
  ...
  async makeTransaction(poapServer: string, event: string) {
    ...
    if (this.alreadyClaimed) {
      ...
      prompt.addText("Already Claimed", 0, 120, Color4.Red(), 24);
    ...
    if (
      !playerStatus.tigerHeadCollectionGameCompleted ||
      !playerStatus.wearableInfoCheckedCompleted
    ) {
      UI.displayAnnouncement(
        `Please complete two missions first to claim this poap`,
        3
      );
      return;
    }
